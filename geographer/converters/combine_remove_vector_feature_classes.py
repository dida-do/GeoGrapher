...
by combining and/or removing vector feature classes.
...
class DSConverterCombineRemoveClasses(DSCreatorFromSource):
    ...
    def _create_or_update(self):
        ...
        classes_to_keep = [
            class_ for list_of_classes in classes
        ...
        if len(self.target_connector.vector_features
        ...
        self.target_connector.add_to_vector_features(features_from_source_df.loc[list(features_to_add_to_target_dataset)])
        ...
        for img_name in tqdm(imgs_to_copy_to_target_dataset, desc="Copying raster images"):
        ...
        if self.label_maker is not None:
            ...
            for img_name in imgs_in_target_dataset_before_addings_imgs_from_source_dataset:
                ...
                if vector_features_intersecting_img & features_to_add_to_target_dataset != set(
                    ...
                    self.label_maker.delete_labels(connector=self.target_connector, img_names=[img_name])
    ...
    def _get_new_class_names(self, classes: List[str]) -> List[str]:
        ...
        if self.new_class_names is None:
            ...
            assert len(new_class_names) == len(set(
                new_class_names)), "new_class_names need to be distinct!"
