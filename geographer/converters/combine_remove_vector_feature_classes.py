...
class DSConverterCombineRemoveClasses(DSCreatorFromSource):
    ...
    label_maker: Optional[LabelMaker] = Field(
        default=None,
        description="Optional LabelMaker. If given, will update labels.")
    ...
    def _create_or_update(self):
        ...
        self.target_connector.task_vector_feature_classes = [
            class_ for class_ in new_class_names
        ...
        features_from_source_df = self._combine_or_remove_classes_from_vector_features(
            ...
            all_source_vector_feature_classes=self.source_connector.
            all_vector_feature_classes,
            classes=classes,
        ...
        if len(
            ...
            self.target_connector.vector_features = empty_vector_features_with_corrected_columns

        self.target_connector.add_to_vector_features(
            features_from_source_df.loc[list(
        ...
        imgs_in_target_dataset_before_addings_imgs_from_source_dataset = {
            img_path.name
            for img_path in self.target_connector.images_dir.iterdir()
        } if self.target_connector.images_dir.exists() else set()
        imgs_in_source_images_dir = {
            img_path.name
            for img_path in self.source_connector.images_dir.iterdir()
        }
        if self.remove_imgs:
            imgs_in_source_that_should_be_in_target = {
                ...
                if (not set(
                    self.source_connector.vector_features_intersecting_img(
                        img_name)).isdisjoint(features_from_source_df.index))
        ...
        self.target_connector.add_to_raster_imgs(
            df_of_imgs_to_add_to_target_dataset)
        ...
        if self.label_maker is not None:
            ...
            for img_name in imgs_in_target_dataset_before_addings_imgs_from_source_dataset:
                ...
                vector_features_intersecting_img = set(
                    self.target_connector.vector_features_intersecting_img(
                        img_name))
        ...
        if self.target_connector.label_type == 'categorical':
            self.target_connector.vector_features.loc[
                features_to_add_to_target_dataset,
                'orig_type'] = self.source_connector.vector_features.loc[
    ...
    def _get_new_class_names(self, classes: List[str]) -> List[str]:
        ...
        if self.new_class_names is None:
            ...
            assert len(new_class_names) == len(
                set(new_class_names)), "new_class_names need to be distinct!"
            assert len(new_class_names) == len(
                classes
    ...
    def _run_safety_checks(self, classes_to_keep: List[str],
        ...
        if self.new_background_class is not None and self.new_background_class not in new_class_names:
            raise ValueError(
                f"new_background_class not in {self.new_class_names}")

    def _combine_or_remove_classes_from_vector_features(
            self, label_type: str, vector_features: GeoDataFrame,
            classes: List[Union[str, List[str]]], new_class_names: List[str],
        ...
        if label_type == 'categorical':
            ...
            vector_features = vector_features.loc[vector_features[
                'type'].apply(lambda class_: class_ in classes_to_keep)]
            ...
            def prob_of_class_names(classes: List[str]) -> List[str]:
                answer = list(
                    map(lambda class_: f"prob_of_class_{class_}", classes))
                return answer

            # drop cols of classes we don't want to keep
            classes_to_drop = [
                class_ for class_ in all_source_vector_feature_classes
            ...
            sum_of_probs_of_remaining_classes = pd.DataFrame(
                vector_features[cols_with_probs_of_remaining_classes].sum(
                    axis=1),
                columns=['sum'],
            ...
            vector_features.loc[:,
                                cols_with_probs_of_remaining_classes] = vector_features[
                                    cols_with_probs_of_remaining_classes].div(
                                        sum_of_probs_of_remaining_classes[
                                            'sum'],
                                        axis=0)

            # combine probabilities of new_classes and drop old classes
            for classes_of_new_class, new_class_name in zip(
                    classes, new_class_names):
                cols_of_probs_to_be_added = prob_of_class_names(
                    classes_of_new_class)
                temp_vector_features[
                    f"prob_of_class_{new_class_name}"] = vector_features[
            ...
            vector_features = GeoDataFrame(
                pd.concat(
                    [vector_features, temp_vector_features],
                    axis=1  # column axis
                ),
