...
class DSConverterGeoTiffToNpy(DSCreatorFromSourceWithBands, ImgBandsGetterMixIn):
    """Convert a dataset of GeoTiffs to NPYs."""
    ...
    channels_first_or_last_in_npy: Literal["last", "first"] = Field(
        default="last",
        description="Ignoring squeezing: 'last' -> (height, width, channels), 'first' -> (channels, height, width).",
    )
    ...
    def _create_or_update(self) -> None:
        ...
        imgs_that_already_existed_in_target_images_dir = {
            img_path.name for img_path in self.target_assoc.images_dir.iterdir()
        }
        ...
        for tif_dir, npy_dir in zip(
            self.source_assoc.image_data_dirs, self.target_assoc.image_data_dirs
        ):
            # ... go through all tif files. ...
            for tif_img_name in tqdm(
                self.source_assoc.raster_imgs.index, desc=f"Converting {tif_dir.name}"
            ):
                # If the corresponding npy in the target image data dir does not exist ...
                if not (npy_dir / self._npy_filename_from_tif(tif_img_name)).is_file():
                    ...
                    with rio.open(tif_dir / tif_img_name) as src:
                        ...
                        seq_extracted_np_bands = [src.read(band) for band in img_bands]
    ...
    def _get_npy_raster_imgs(self):
        ...
        tif_img_name_list = (
            npy_raster_imgs.index.tolist().copy()
