...
class SingleImgCutterAroundFeature(SingleImgCutter):
        ...
        **kwargs,
        ...
        if mode in {"random", "centered"}:
            if new_img_size is None:
                raise ValueError(f"if mode is {mode}, new_img_size should not be None")
    ...
    def _check_img_size_type_and_value(self, img_size: ImgSize):
        ...
        if not isinstance(img_size, int) or (
            isinstance(img_size, tuple)
            and len(img_size) == 2
            and all(isinstance(entry, int) for entry in img_size)
        ):
            raise TypeError(
        ...
        if not img_size_rows > 0:
            logger.error(
                "new_img_size is %s, need to have positive side length(s)", img_size
            )
    ...
    def _get_windows_transforms_img_names(
        ...
        **kwargs: Any,
        ...
        feature_geom = target_connector.vector_features.loc[feature_name, "geometry"]
        ...
        with rio.open(source_img_path) as src:
            ...
            assert source_connector.vector_features.loc[feature_name].geometry.within(
            ...
            (
                row_off,
                col_off,
                num_small_imgs_in_row_direction,
                num_small_imgs_in_col_direction,
            ...
            for img_row in range(num_small_imgs_in_row_direction):
                for img_col in range(num_small_imgs_in_col_direction):
                    ...
                    if (
                        num_small_imgs_in_row_direction == 1
                        and num_small_imgs_in_col_direction == 1
                    ):
                    ...
                    if window_bounding_rectangle.intersects(transformed_feature_geom):
    ...
    def _get_min_max_row_col(
        ...
        list_of_enveloping_rectangle_row_col_pairs = list(
            map(lambda pair: img.index(*pair), list_of_rectangle_corner_coords)
        )
    ...
    def _get_grid_row_col_offsets_num_windows_row_col_direction(
        ...
        if self.mode == "random":
            ...
            row_off = random.randint(
                max(0, max_row - new_img_size_rows * num_small_imgs_in_row_direction),
                min(
                    min_row,
                    img.height - new_img_size_rows * num_small_imgs_in_row_direction,
                ),
            )

            col_off = random.randint(
                max(0, max_col - new_img_size_cols * num_small_imgs_in_col_direction),
                min(
                    min_col,
                    img.width - new_img_size_cols * num_small_imgs_in_col_direction,
                ),
            )
            ...
            row_off = centroid_row - (
                new_img_size_rows * num_small_imgs_in_row_direction // 2
            )
            col_off = centroid_col - (
                new_img_size_cols * num_small_imgs_in_col_direction // 2
            )
