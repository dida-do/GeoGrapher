...
class CreateDSCategoricalFromSoftCategoricalDatasetMixIn(object):
    ...
    def _update_categorical_from_soft_categorical_dataset(self) -> None:
        """Update an existing dataset created using
        create_categorical_from_soft_categorical_dataset.
    ...
    def _create_or_update_categorical_from_soft_categorical_dataset(
        ...
        """Create a new dataset of GeoTiffs (and associator) with categorical
        labels from an existing dataset with soft-categorical labels by taking
        label with the highest probability (with random tiebreaking).
        ...
        if create_or_update == 'update':
            ...
            if not target_assoc.label_type == 'categorical':
                raise ValueError(
                    f"label_type of target dataset should be 'categorical', but is {target_assoc.label_type}"
                )
        ...
        polygons_that_will_be_added_to_target_dataset = set(
            self.polygons_df.index) - set(target_assoc.polygons_df.index)
        ...
        source_polygons_df_converted_to_soft_categorical_format = convert_polygons_df_soft_cat_to_cat(
            self.polygons_df)
        target_assoc.add_to_polygons_df(
        ...
        imgs_that_already_existed_in_target_dataset = {
            img_name
            for img_name in target_assoc.imgs_df.index
            if (target_assoc.images_dir / img_name).is_file()
        }
        imgs_in_source_images_dir = {
            img_name
            for img_name in self.imgs_df.index
        }
        ...
        target_assoc._update_from_source_dataset_dict.update({
            'update_method':
            '_update_categorical_from_soft_categorical_dataset',
            'source_data_dir':
            self.images_dir.
            parent,  # !!! Assuming standard directory format here.
        })
        target_assoc.save()

        log.info(
            f"_convert_or_update_dataset_soft_categorical_to_categorical: done!"
        )
