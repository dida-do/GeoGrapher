...
class CreateDSTiffToNpyMixIn(object):
    ...
    def convert_dataset_from_tif_to_npy(
        ...
        return self._convert_or_update_dataset_from_tif_to_npy(
            ...
            squeeze_label_channel_dim_if_single_channel=
            squeeze_label_channel_dim_if_single_channel,
    ...
    def _update_dataset_converted_from_tif_to_npy(
        ...
        squeeze_label_channel_dim_if_single_channel = self._update_from_source_dataset_dict[
            'squeeze_label_channel_dim_if_single_channel']
        channels_first_or_last_in_npy = self._update_from_source_dataset_dict[
            'channels_first_or_last_in_npy']
    ...
    def _convert_or_update_dataset_from_tif_to_npy(
        ...
        non_tif_imgs = list(
            filter(lambda s: not s.endswith('.tif'), self.imgs_df.index))
        if non_tif_imgs != []:
        ...
        for tif_dir, npy_dir in zip(self.image_data_dirs,
                                    target_assoc.image_data_dirs):
            # ... go through all tif files. ...
            for tif_img_name in tqdm(tif_img_name_list,
                    ...
                    with rio.open(tif_dir / tif_img_name) as src:
                        ...
                        if str(tif_dir.name) == "images":
                            if img_bands is None:
                                bands = list(range(1, src.count +
                                                   1))  # default to all bands
                            else:
                                bands = img_bands
                        ...
                        seq_extracted_np_bands = [
                            src.read(band) for band in bands
                        ]

                        # new img path
                        new_npy_img_path = npy_dir / tif_filename_to_npy(
                            tif_img_name)
                        ...
                        if str(
                                tif_dir.name
                        ) == "labels" and squeeze_label_channel_dim_if_single_channel == True:
                            if len(bands) == 1:
        ...
        target_assoc._update_from_source_dataset_dict.update({
            'update_method':
            '_update_dataset_converted_from_tif_to_npy',
            'source_data_dir':
            self.images_dir.
