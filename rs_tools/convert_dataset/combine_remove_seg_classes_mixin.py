...
class CreateDSCombineRemoveSegClassesMixIn(object):
    ...
    def create_dataset_by_combining_or_removing_seg_classes(
        ...
        Example:
            ...
            Suppose the associator assoc's dataset has the following segmentation classes (including the background class 'bg' and class 'ig'):
    ...
    def _update_dataset_by_combining_or_removing_seg_classes(self) -> ImgPolygonAssociator:
        """
        ...
        new_seg_classes = self._update_from_source_dataset_dict['new_seg_classes']
        class_separator = self._update_from_source_dataset_dict['class_separator']
        new_background_class = self._update_from_source_dataset_dict['new_background_class']
    ...
    def _create_or_update_dataset_by_combining_or_removing_seg_classes(
        ...
        if not set(classes_to_keep) <= set(self.all_polygon_classes):
            classes_not_in_source_dataset = set(classes_to_keep) - set(self.all_polygon_classes)
            raise ValueError("The following classes are not in self.all_polygon_classes: {classes_not_in_source_dataset}")
        ...
        if create_or_update == 'create' and target_assoc.label_type == 'soft-categorical':
            empty_polygons_df_with_corrected_columns = self._combine_or_remove_seg_classes_from_polygons_df(
                                        ...
                                        seg_classes=seg_classes,
                                        new_seg_classes=new_seg_classes,
                                        all_polygon_classes=self.all_polygon_classes # self, since we already set classes in target_assoc
            )
        ...
        if remove_imgs:
            imgs_in_source_that_should_be_in_target = {
                                                        ...
                                                        img_name for img_name in imgs_in_source_images_dir
                                                            ...
                                                            if set(self.polygons_intersecting_img(img_name)) & set(polygons_from_source_df.index) != set()
                                                    }
        else:
        ...
        df_of_imgs_to_add_to_target_dataset = self.imgs_df.loc[list(imgs_to_copy_to_target_dataset)]
