...
class CreateDSCutEveryImgToGridMixIn(object):
    ...
    def cut_every_img_to_grid(
        ...
        """Create a new dataset of GeoTiffs (images, labels, and associator)
        where each image is cut into a grid of images.
    ...
    def _update_cut_every_img_to_grid(self) -> None:
        """Update a dataset of GeoTiffs (images, labels, and associator) where
        each image is cut into a grid of images.
        ...
        required_cut_params = {
            'source_data_dir', 'new_img_size', 'img_bands', 'label_bands'
        }

        if not required_cut_params <= set(
                self._update_from_source_dataset_dict.keys()):
            raise KeyError(
                f"The following cut params are missing: {set(self._update_from_source_dataset_dict.keys()) - required_cut_params}"
            )

        self._create_or_update_cut_every_img_to_grid(
            create_or_update='update',
            source_data_dir=self.
            _update_from_source_dataset_dict['source_data_dir'],
