...
class CreateDSCutImgsAroundEveryPolygonMixIn(object):
    ...
    def _update_cut_imgs_around_every_polygon(self) -> None:
        """Update a dataset of GeoTiffs created by
        cut_imgs_around_every_polygon.
        ...
        required_cut_params = {
            'mode', 'source_data_dir', 'new_img_size', 'min_new_img_size',
            'scaling_factor', 'target_img_count', 'img_bands', 'label_bands',
            'random_seed'
        }

        if not required_cut_params <= set(
                self._update_from_source_dataset_dict.keys()):
            raise KeyError(
                f"Can't update, the following cut params are missing: {set(self._update_from_source_dataset_dict.keys()) - required_cut_params}"
            )
        ...
        self._create_or_update_cut_imgs_around_every_polygon(
            ...
            min_new_img_size=self.
            _update_from_source_dataset_dict['min_new_img_size'],
            scaling_factor=self.
            _update_from_source_dataset_dict['scaling_factor'],
            target_img_count=self.
            _update_from_source_dataset_dict['target_img_count'],
    ...
    def _create_or_update_cut_imgs_around_every_polygon(
            ...
            img_bands: Optional[List[int]] = None,
            label_bands: Optional[List[int]] = None,
            random_seed: int = 10) -> Optional[ImgPolygonAssociator]:
        """Create or update a dataset of GeoTiffs so that it contains (if
        possible) for each polygon in the target (or source) dataset a number
        target_img_count of images cut from images in the source dataset.
        ...
        target_assoc._update_from_source_dataset_dict.update({
            'update_method':
            '_update_cut_imgs_around_every_polygon',
            'mode':
            mode,
            'source_data_dir':
            source_data_dir,
