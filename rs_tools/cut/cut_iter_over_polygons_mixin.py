...
from typing import Any, Callable, DefaultDict, Dict, Set, Union, List, Optional, Hashable, TYPE_CHECKING
import logging
...
class CreateDSCutIterOverPolygonsMixIn(object):
    ...
    def create_or_update_dataset_iter_over_polygons(
        ...
        if create_or_update == 'create':
            ...
            target_assoc._update_from_source_dataset_dict['cut_imgs'] = defaultdict(list)
        ...
        # For each polygon ...
        for polygon_name in tqdm(target_assoc.polygons_df.index, desc='Cutting dataset: '): #!!!!!!!!! all_polygons?????????
        ...
        target_assoc._update_from_source_dataset_dict['cut_imgs'] = map_dict_values(
                                                                        remove_duplicates,
                                                                        target_assoc._update_from_source_dataset_dict['cut_imgs'])
    ...
    def _filter_out_previously_cut_imgs(
        ...
        src_imgs_previously_cut_for_this_polygon = set(target_assoc._update_from_source_dataset_dict['cut_imgs'][polygon_name])
        answer = list(src_imgs_containing_polygon - src_imgs_previously_cut_for_this_polygon)
