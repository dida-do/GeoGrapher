"""Mix-in that implements the private methods used to manipulate a connector's bipartite graph."""
...
class BipartiteGraphMixIn:
    ...
    def have_img_for_feature(self, feature_name: str) -> bool:
        ...
        Returns:
            bool: `True` if there is an image in the dataset fully containing the vector feature, False otherwise.
        """
    ...
    def rectangle_bounding_img(self, img_name: str) -> BaseGeometry:
        ...
        Returns:
            BaseGeometry: shapely geometry giving the bounds of the image in the standard crs of the connector
        """

        return self.raster_imgs.loc[img_name, 'geometry']

    def features_intersecting_img(
        ...
        Returns:
            list of strs of feature_names/ids of all vector features in connector which have non-empty intersection with the image(s)
        """
    ...
    def imgs_intersecting_feature(self,
        ...
        Returns:
            list of str: list of the feature_names/identifiers of all vector features in connector with non-empty intersection with the image.
        """
    ...
    def features_contained_in_img(
        ...
        Returns:
            list of str: list of the feature_names/identifiers of all vector features in connector contained in the image(s).
        """
    ...
    def does_img_contain_feature(self, img_name: str,
        ...
        Returns:
            bool: True or False depending on whether the image contains the vector feature or not
        """

        return feature_name in self.features_contained_in_img(img_name)

    def is_feature_contained_in_img(self, feature_name: str,
                                    img_name: str) -> bool:
        """
        Args:
            img_name (str): Name of image
        ...
        Returns:
            bool: True or False depending on whether the vector feature contains the image or not
        """

        return self.does_img_contain_feature(img_name, feature_name)

    def does_img_intersect_feature(self, img_name: str,
                                   feature_name: str) -> bool:
        """
        Args:
            img_name (str): Name of image
        ...
        Returns:
            bool: True or False depending on whether the image intersects the vector feature or not
        """

        return feature_name in self.features_intersecting_img(img_name)

    def does_feature_intersect_img(self, feature_name: str,
                                   img_name: str) -> bool:
        """
        Args:
            img_name (str): Name of image
        ...
        Returns:
            bool: True or False depending on whether the vector feature intersects the image or not
        """

        return self.does_img_intersect_feature(img_name, feature_name)

    def _connect_img_to_feature(
            self,
            img_name: str,
            feature_name: str,
        ...
        Remember (i.e. create a connection in the graph) whether the image fully contains or just has non-empty intersection with the vector feature, i.e. add an edge of the approriate type between the image and the vector feature.

        Args:
            img_name (str): Name of image to connect
            feature_name (str): Name of vector feature to connect
            contains_or_intersects (optional, str): Optional connection criteria
            vector_features (optional, gdf.GeoDataFrame): Optional vector feature dataframe
            img_bounding_rectangle (optional, BaseGeometry): vector feature decribing image footprint
    ...
    def _add_feature_to_graph(self,
                              feature_name: str,
                              vector_features: Optional[GeoDataFrame] = None):
        """Connects a vector feature to those images in self.raster_imgs with which it has
        non-empty intersection.

        Args:
            feature_name (str): name/id of vector feature to add
            vector_features (GeoDataFrame, optional): Defaults to None (i.e. self.vector_features).
        """
        ...
        for img_name in containing_imgs:
            self._connect_img_to_feature(img_name,
                                         feature_name,
                                         'contains',
    ...
    def _add_img_to_graph_modify_vector_features(
        ...
        img_counts in vector_features where appropriate. The default values None
        for vector_features and graph will be interpreted as self.vector_features and
        self.graph. If img_bounding_rectangle is None, we assume we can get it
        from self. If the image already exists and already has connections a
        warning will be logged.

        Args:
            img_name (str): Name of image to add
            img_bounding_rectangle (optional, BaseGeometry): vector feature decribing image footprint
            vector_features (optional, gdf.GeoDataFrame): Optional vector features dataframe
            graph (optional, BipartiteGraph): optional bipartied graph
        """
    ...
    def _remove_feature_from_graph_modify_vector_features(
            self, feature_name: str, set_img_count_to_zero: bool = True):
        """Removes a vector feature from the graph (i.e. removes the vertex and all
        incident edges) and (if set_img_count_to_zero == True) sets the
        vector_features field 'img_count' to 0.

        Args:
            feature_name (str): vector feature name/id
    ...
    def _remove_img_from_graph_modify_vector_features(self, img_name: str):
        ...
        incident edges) and modifies the vector_features fields 'img_count' for the
        vector features contained in the image.

        Args:
            img_name (str): name/id of image to remove
        """

        for feature_name in self.features_contained_in_img(img_name):
