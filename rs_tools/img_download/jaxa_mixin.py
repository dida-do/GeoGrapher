"""data_version '1804' jaxa_file_name 'S034E152.tar.gz' jaxa_folder_name
'S035E150/'.
...
class JAXADownloaderMixIn(object):
        ...
        for jaxa_specific_keyword_arg, value in {
            ...
            if value is None:
                try:
                    ...
                    raise ValueError(
                        f"Need to set {jaxa_specific_keyword_arg} keyword argument for the jaxa downloader."
                    )
            else:
                # Remember value
        ...
        for jaxa_file_name, jaxa_folder_name in jaxa_file_and_folder_names:
            ...
            if jaxa_file_name[:
                ...
                try:
                    ...
                    log.warning(
                        f'File {jaxa_file_name} in folder {jaxa_folder_name} could not be found on JAXA ftp or could not be opened: {e.args}'
                    )
                    # continue
                    # download_exception = 'file_not_available_on_JAXA_ftp'
                    ...
                    os.remove(os.path.join(download_dir,
                                           jaxa_file_name))  # delete .tar file
                    # move needed DEM-file outside of unzipped folder (existing versions are overwritten)
                    shutil.move(
                        str(
                            Path(download_dir) / jaxa_file_name[:-7] /
                            (jaxa_file_name[:-7] + '_AVE_DSM.tif')),
                        str(
                            Path(download_dir) /
                            (jaxa_file_name[:-7] + '_DSM.tif')))
                    os.rmdir(
                        os.path.join(Path(download_dir), jaxa_file_name[:-7])
                    )  # remove folder, only works when it is empty
                    ...
                    list_img_info_dicts.append({
                        'img_name':
                        jaxa_file_name[:-7] +
                        '_DSM.tif',  # TODO: check relevance of _AVE_ file-name component, consider to rename/remove
                        'img_processed?':
                        False,
                        'timestamp':
    ...
    def _process_downloaded_img_file_jaxa(self, img_name: str,
                                          in_dir: Union[Path, str],
                                          out_dir: Union[Path, str],
                                          convert_to_crs_epsg: int, **kwargs):
        """provides required postprocessing for downloaded jaxa DEM. Currently
        only copying from in_dir to out_dir is performed.
        ...
        img_info_dict = {
            'orig_crs_epsg_code':
            orig_crs_epsg_code,  # alternatively directly use 4326 for WGS84 (used for jaxa)
            'img_name':
            img_name,
            'img_processed?':
            True,
            'geometry':
            transform_shapely_geometry(img_bounding_rectangle,
                                       orig_crs_epsg_code, 4326)
        }
