...
class Sentinel2DownloaderMixIn:
    ...
    def _download_imgs_for_polygon_sentinel2(
        ...
        if producttype in {'L2A', 'S2MSI2A'}:
            ...
            raise LookupError(
                f"ImgPolygonAssociator.__init__: unknown producttype: {producttype}"
            )
        ...
        raise NoImgsForPolygonFoundError(
            f"Either no images were found for {polygon_name} found or all images failed to download."
        )
    ...
    def _process_downloaded_img_file_sentinel2(self, img_name: str,
        ...
        orig_crs_epsg_code, img_bounding_rectangle_orig_crs = conversion_dict[
            "crs_epsg_code"], conversion_dict["img_bounding_rectangle"]
