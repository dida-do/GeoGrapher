...
def _make_geotif_label_onehot(assoc, img_name, log):
    ...
    Create a categorical one-hot encoded (i.e. one channel per segmentation class, each entry is either zero or not, and at each coordinate in the image exactly one channel has a non-zero entry) GeoTiff raster label in the data directory's labels subdirectory for a GeoTiff image with image name img_name. 
            ...
            with rio.open(label_path,
                ...
                for count, seg_class in enumerate(assoc.__params_dict__['segmentation_classes'], start=1):
                    ...
                    # Else, burn the values for those polygons into the band.
                        ...
                        mask = rio.features.rasterize(shapes=polygon_geometries_in_src_crs, 
