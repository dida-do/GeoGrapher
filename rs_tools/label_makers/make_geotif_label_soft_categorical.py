...
def _make_geotif_label_soft_categorical(assoc, img_name, log):
    """
    Create a soft categorical (i.e. probabilistic) GeoTiff pixel label for an image.

    Create a soft-categorical encoded (i.e. one channel per segmentation class, the at a given position in a given channel is the probability that the pixel at that position is classified as belonging to the corresponding segmentation class) GeoTiff raster label in the data directory's labels subdirectory for a GeoTiff image with image name img_name. Assumes the associator's polygons_df contains one column prob_seg_class_[segmentation_class] for each segmentation class containing the probability of that polygon belonging to the segmentation class. 

    Args:
        - assoc: calling ImgPolygonAssociator.
        - img_name (img_name): Name of image for which a label should be created. 
    ...
    if not img_path.is_file():
        ...
        # ... log error to file.
        log.error(f"_make_geotif_label_soft_categorical: input image {img_path} does not exist!")

    # Else, if the label already exists ...
    elif label_path.is_file():

        # ... log error to file.
        log.error(f"_make_geotif_label_soft_categorical: label {label_path} already exists!")
        ...
        with rio.open(img_path) as src:
            ...
            with rio.open(label_path,
                ...
                for count, seg_class in enumerate(segmentation_classes, start=1):
                    ...
                    polygon_geometries_in_std_crs = list(polygons_intersecting_img_df['geometry'])

                    # ... and convert them to the crs of the source image. 
                    polygon_geometries_in_src_crs = list(map(
                                                            lambda geom: transform_shapely_geometry(geom, 
                                                                                                    assoc.polygons_df.crs.to_epsg(), 
                                                                                                    src.crs.to_epsg()), 
                                                            polygon_geometries_in_std_crs)) 

                    # Extract the class probabilities ...
                    class_probabilities = list(polygons_intersecting_img_df[f"prob_seg_class_{seg_class}"])

                    # .. and combine with the polygon geometries to a list of (polygon, value) pairs.
                    polygon_value_pairs = list(zip(polygon_geometries_in_src_crs, 
                                                    class_probabilities))
                    ...
                    # Else, burn the values for those polygons into the band.
                        ...
                        mask = rio.features.rasterize(shapes=polygon_value_pairs, 
                ...
                if assoc._params_dict['add_background_band_in_labels'] == True:
                    ...
                    non_background_band_indices = list(range(1, 1 + len(segmentation_classes)))
