...
NEW_TARGET_DATA_DIR = Path.home() / Path("rstools/pytest_data_dirs/") / Path("target")

# for test_new_dataset_one_small_img_for_each_polygon
UPDATE_TARGET_DATA_DIR = Path.home() / Path("rstools/pytest_data_dirs/") / Path("update_target")
COPY_UPDATE_TARGET_DATA_DIR = Path.home() / Path("rstools/pytest_data_dirs/") / Path("update_target_copy")
...
CORRECT_TARGET_DIR = Path.home() / Path("rstools/pytest_data_dirs/") / Path("correct")

# for test_not_centered_cutting 
NOT_CENTERED_SOURCE_DATA_DIR = Path.home() / Path("rstools/pytest_data_dirs/") / Path("source_centered_false_test")
# (path of) data dir that will be created
NOT_CENTERED_TARGET_DATA_DIR = Path.home() / Path("rstools/pytest_data_dirs/") / Path("centered_false_target_copy")
...
def test_not_centered_cutting():
    ...
    for n in range(4):
        ...
        with rio.open(target_data_dir / Path('labels') / Path('S2A_MSIL2A_20200403T143721_N0214_R096_T19KES_20200403T184818_730.tif')) as src: 
            ...
            raster = src.read(1) # polygon 730 is a  object, so the non-default (non-zero) values of the label are 2.
