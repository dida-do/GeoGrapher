...
WARNING! Uses real data in AuBeSa folder on markov. Ideally, should use artificially generated data. 
"""
...
SOURCE_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("source")
NEW_TARGET_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("target")

# for test_new_dataset_one_small_img_for_each_polygon
UPDATE_TARGET_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("update_target")
COPY_UPDATE_TARGET_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("update_target_copy")
...
CORRECT_TARGET_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("correct")

# for test_not_centered_cutting 
NOT_CENTERED_SOURCE_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("source_centered_false_test")
# (path of) data dir that will be created
NOT_CENTERED_TARGET_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path("centered_false_target_copy")


def test_new_dataset_one_small_img_for_each_polygon():
    """
    Create a new dataset from an old one using cut.new_dataset_one_small_img_for_each_polygon. The source dataset contains both polygons without images and polygons that will be contained in the same image.     
    """
...
def test_not_centered_cutting():
    """
    Small_imgs_centered_around_polygons_splitter in cut_dataset_utils has a "centered" argument. If False, the window around the image being cut should have random offsets subject to the contraint that it should fully contain the polygon under consideration. This test is to make sure that this constraint is always satisfied.
    """
    ...
    for n in range(4):
        ...
        with rio.open(target_data_dir / Path('labels') / Path('S2A_MSIL2A_20200403T143721_N0214_R096_T19KES_20200403T184818_730.tif')) as src: 
            ...
            raster = src.read(1) # polygon 730 is a cross valley tailing, so the non-default (non-zero) values of the label are 2.
            ...
            #### Once I got {0: 904627, 2: 143949}? Somehow this can fail if the tailing is at the edge of the picture... ???
