...
Simple pytest test suite of rstoolsImgPolygonAssociator using artificial data (i.e. we write a polygons_df and an raster_imgs by hand and don't test any methods (e.g. add/download missing images, create labels) that deal with the actual images.)
...
def test_img_polygon_associator_rstools():
    ...
    assert list(assoc.imgs_containing_geom('polygon1')) == ['img1']
    assert list(assoc.geoms_contained_in_img('img2')) == []
    assert list(
        assoc.geoms_intersecting_img('img2')) == ['polygon1', 'polygon3']
    assert list(
        assoc.geoms_intersecting_img('img1')) == ['polygon1', 'polygon2']
    ...
    polygon3 = box(
        -100, -100, -99,
        -99)  # entry for polygon3 already exists in assoc.polygons_df
