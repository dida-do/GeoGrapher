...
Simple pytest test suite of AuBeSaImgPolygonAssociator using artificial data (i.e. we write a polygons_df and an imgs_df by hand and don't test any methods (e.g. add/download missing images, create labels) that deal with the actual images.)
...
def test_img_polygon_associator_aubesa():
    ...
    empty_imgs_df = ipa.empty_imgs_df(
    ...
    assert list(
        assoc.polygons_intersecting_img('img2')) == ['polygon1', 'polygon3']
    assert list(
        assoc.polygons_intersecting_img('img1')) == ['polygon1', 'polygon2']
