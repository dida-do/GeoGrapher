...
def test_bipartite_graph():
    ...
    for vertex in red_vertices:
        assert graph1.exists_vertex(vertex, "red")

    for vertex in black_vertices:
        assert graph1.exists_vertex(vertex, "black")
    ...
    assert graph1._graph_dict == {
        "red": {"r3": {"b1": "bar"}, "r2": {}, "r1": {"b1": "foo", "b2": "foo"}},
        "black": {"b3": {}, "b2": {"r1": "foo"}, "b1": {"r1": "foo", "r3": "bar"}},
    }
    ...
    with pytest.raises(Exception):
        graph1.add_edge(
            "r1", "red", "b1", "foo"
        )  # edge was in edges_with_existing_vertices

    # test adding edges with only one existing vertex which is
    # of the same color as the source vertex
    ...
    for (
        ...
        to_vertex,
        edge_data,
    ) in edges_with_one_existing_vertices_of_source_color:
        graph1.add_edge(from_vertex, from_color, to_vertex, edge_data)
    ...
    assert graph1._graph_dict == {
        "red": {
            "r3": {"b1": "bar", "b6": "bar"},
            "r2": {},
            "r1": {"b1": "foo", "b2": "foo", "b4": "foo", "b5": "foo"},
        },
        "black": {
            "b3": {},
            "b1": {"r1": "foo", "r3": "bar"},
    ...
    # test adding edges with only one existing vertex which is
    # of the opposite color as the source vertex
    ...
    for (
        ...
        to_vertex,
        edge_data,
    ) in edges_with_one_existing_vertices_of_opposite_color:
        graph1.add_edge(from_vertex, from_color, to_vertex, edge_data)
    ...
    for (
        ...
        to_vertex,
        edge_data,
    ) in edges_without_existing_vertices:
        graph1.add_edge(from_vertex, from_color, to_vertex, edge_data)
    ...
    for from_vertex, from_color, to_vertex, edge_data in edges_with_existing_vertices:
        graph1.add_edge(from_vertex, from_color, to_vertex, edge_data, force=True)
    ...
    assert set(graph1.vertices_opposite("b4", "black")) == set({"r1", "r4", "r5"})
    # test with filtered edge_data: replace edge_data on edge b4 -> r1 to "bar"
    graph1.add_edge("b4", "black", "r1", "bar", force=True)
    assert set(graph1.vertices_opposite("b4", "black", edge_data="foo")) == set(
        {"r4", "r5"}
    )
    assert set(graph1.vertices_opposite("b4", "black", edge_data="bar")) == set({"r1"})
    ...
    assert graph1._graph_dict == {
        "red": {"r3": {"b1": "foo"}, "r2": {}, "r1": {"b1": "foo", "b2": "foo"}},
        "black": {"b3": {}, "b2": {"r1": "foo"}, "b1": {"r1": "foo", "r3": "foo"}},
    }
    ...
    assert graph1._graph_dict == {
        "red": {"r3": {}, "r2": {}, "r1": {"b1": "foo", "b2": "foo"}},
        "black": {"b3": {}, "b2": {"r1": "foo"}, "b1": {"r1": "foo"}},
    }

    # test really_undirected
    assert graph1.really_undirected()
