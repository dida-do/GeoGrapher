...
real data in AuBeSa folder on markov. Ideally, should use artificially
generated data.
"""
...
SOURCE_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path(
    "source")
NEW_TARGET_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path(
    "target")

# for test_new_dataset_one_small_img_for_each_polygon
UPDATE_TARGET_DATA_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path(
    "update_target")
COPY_UPDATE_TARGET_DATA_DIR = Path.home() / Path(
    "AuBeSa/pytest_data_dirs/") / Path("update_target_copy")
...
CORRECT_TARGET_DIR = Path.home() / Path("AuBeSa/pytest_data_dirs/") / Path(
    "correct")

# for test_not_centered_cutting
NOT_CENTERED_SOURCE_DATA_DIR = Path.home() / Path(
    "AuBeSa/pytest_data_dirs/") / Path("source_centered_false_test")
# (path of) data dir that will be created
NOT_CENTERED_TARGET_DATA_DIR = Path.home() / Path(
    "AuBeSa/pytest_data_dirs/") / Path("centered_false_target_copy")
...
def test_not_centered_cutting():
    ...
    for n in range(4):
        ...
        with rio.open(target_data_dir / Path('labels') / Path(
            ...
            raster = src.read(
                1
            )  # polygon 730 is a cross valley tailing, so the non-default (non-zero) values of the label are 2.
