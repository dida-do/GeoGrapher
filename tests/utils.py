...
def create_dummy_imgs(data_dir: Union[Path, str], img_size: int) -> None:
    ...
    for img_name, bbox_geom, epsg_code in connector.raster_imgs[["geometry", "orig_crs_epsg_code"]].itertuples():
        img_array = np.stack(np.ones((img_size, img_size), dtype=np.uint8) * n for n in range(3))
        bbox_geom_in_img_crs = transform_shapely_geometry(bbox_geom, from_epsg=connector.crs_epsg_code, to_epsg=epsg_code)
        transform = rio.transform.from_bounds(*bbox_geom_in_img_crs.bounds, img_size, img_size)
